...
export async function createRepo(
  ...
  if (protocol === "subduction") {
    // A single websocket connection — every repo.find multiplexes over it,
    // so large-doc clones are bound by that one socket's throughput, not by
    // client concurrency. Parallel sockets come from shard mode
    // (PUSHWORK_PARALLEL_INGEST), where each worker thread owns a separate
    // Repo/SubductionSource/socket. (See CLAUDE.md for the rejected
    // single-process multi-socket approaches.)
