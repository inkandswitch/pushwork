 ...
 *
 * Everything here is a no-op unless profiling is enabled (via the
 * `--profile` flag or `PUSHWORK_PROFILE=1`), so the hot-path wrappers add
 * a single boolean check in normal runs.
 *
 * Output goes to **stderr** so it never contaminates a command's stdout
 * (e.g. the root URL printed at the end of `sync`). A single machine
 * readable `PROFILE_JSON { ... }` line is emitted for the bench harness.
 */
 ...
 * timing; a tick arriving `>= thresholdMs` late means the loop was
 * blocked. Also samples RSS each tick to capture peak memory. The timer
